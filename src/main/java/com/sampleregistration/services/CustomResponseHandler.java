...
public class CustomResponseHandler implements ResponseHandler {
	...
	public ResponseObject handleResponse(final HttpResponse response) throws ClientProtocolException, IOException {
		...
		if (status >= 200 && status < 300) {
			...
			if (obj != null && (obj.toString().equals("true") || obj.toString().equals("false"))) {
				rspObject.setSuccess((boolean) jsonObject.get("success"));
			} else {
				...
				rspObject.setDeletehash((String) dataObject.get("deletehash"));
				rspObject.setSuccess((boolean) dataObject.get("success"));
			}
		}
		return rspObject;
	}
